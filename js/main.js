...
(function () {
  ...
  if (capc) {
    ...
    function capCompute() {
      ...
      var behoefte = dag * 0.55; // deel van dagverbruik dat je 's avonds/'s nachts uit de batterij haalt
      var advies;
      if (s.zon === 'ja' && s.panelen > 0) {
        var opwekDag = s.panelen * 0.42 * 2.7;   // ~0,42 kWp/paneel, ~2,7 kWh per kWp per dag (NL-gemiddelde)
        var opslaanbaar = opwekDag * 0.55;        // overschot dat je daadwerkelijk kunt opslaan
        advies = Math.min(behoefte, opslaanbaar);
      } else {
        advies = behoefte * 0.65;                 // zonder (huidige) opwek: kleinere richtmaat
      }
      if (s.extra === 'ja') advies = advies * 1.25; // warmtepomp/EV verhoogt de behoefte
      advies = Math.max(3, Math.min(advies, s.extra === 'ja' ? 25 : 20));
      var adviesR = Math.round(advies);
      var onder = Math.max(2, adviesR - 2);
      var boven = adviesR + 2;
      ...
      setOut('dag', 'Gemiddeld dagverbruik: ' + nl(dag, 1) + ' kWh');
    }
