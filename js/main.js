...
(function () {
  ...
  /* Op mobiel: op de offertepagina automatisch zacht naar de calculator scrollen,
     zodat bezoekers het formulier meteen zien (niet alleen de introtekst). */
  (function autoScrollOfferte() {
    var isOffertePagina = /offerte\.html$/.test(location.pathname) || /\/offerte\/?$/.test(location.pathname);
    if (!isOffertePagina) return;
    var isMobiel = window.matchMedia && window.matchMedia('(max-width: 760px)').matches;
    if (!isMobiel) return;
    var mount = document.querySelector('.calc-mount');
    if (!mount) return;
    // niet doen als er via een anker of eerdere scrollpositie al gescrold is
    if (window.scrollY > 40 || location.hash) return;
    var tries = 0;
    function go() {
      tries++;
      var calc = mount.querySelector('.calc') || mount;
      var top = calc.getBoundingClientRect().top + window.scrollY - 96; // ruimte voor de vaste kopbalk
      if (top > 40) {
        window.scrollTo({ top: top, behavior: 'smooth' });
      } else if (tries < 6) {
        setTimeout(go, 200); // calculator nog aan het renderen: nog even wachten
      }
    }
    // korte vertraging zodat de calculator gerenderd is en de lay-out gezet is
    setTimeout(go, 450);
  })();
})();
