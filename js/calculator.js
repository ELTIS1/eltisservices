...
(function () {
  ...
  var Q_WONING = { key: 'woning', label: 'Wat voor woning heb je?', type: 'pills', required: true,
    options: ['Vrijstaand', '2-onder-1-kap', 'Rijwoning', 'Appartement', 'Bedrijfspand'] };
  ...
  var branch = {
    ...
    laadpaal: [
      Q_WONING,
      { key: 'aansluiting', label: 'Wat voor aansluiting heb je?', hint: 'staat op je meterkast', type: 'pills', required: true, options: ['1-fase', '3-fase', 'Weet ik niet'] },
      { key: 'vermogen', label: 'Gewenst laadvermogen', type: 'pills', required: true, options: ['Adviseer mij', '3,7 kW', '7,4 kW (1-fase)', '11 kW (3-fase)', '22 kW'] },
      { key: 'montage', label: 'Waar komt de laadpaal?', type: 'pills', required: true, options: ['Aan de muur (wallbox)', 'Op een paal'] },
      ...
      { key: 'kmjaar', label: 'Hoeveel rijd je per jaar?', hint: 'optioneel', type: 'select',
        options: ['Minder dan 10.000 km', '10.000 – 20.000 km', '20.000 – 30.000 km', 'Meer dan 30.000 km'] }
    ]
  };

  var labels = {
    woning: 'Woning', zon: 'Zonnepanelen', wp: 'Wattpiek', verbruik: 'Jaarverbruik', aansluiting: 'Aansluiting',
    contract: 'Dynamisch contract', capaciteit: 'Capaciteit', vermogen: 'Laadvermogen', montage: 'Montage',
    afstand: 'Afstand', balancing: 'Load balancing', kmjaar: 'Kilometers/jaar'
  };
  ...
  function buildCalc(mount) {
    ...
    root.innerHTML =
      '<div class="calc-head">' +
        ...
        '<div class="calc-progress"><div class="fill"></div></div>' +
      '</div>' +
    ...
    function setProgress(done) {
      var pct = done ? 100 : Math.round(((stepIndex + 1) / TOTAL) * 100);
      fill.style.width = pct + '%';
      stepc.textContent = done ? 'Klaar' : 'Stap ' + (stepIndex + 1) + ' / ' + TOTAL;
    }
