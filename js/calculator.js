...
(function () {
  ...
  // >>> CONFIGUREER HIER <<<
  var FORMSPREE_ENDPOINT = 'https://formspree.io/f/mwvgbbyy';
  ...
  // ---- funnel-definitie ----
  var services = {
    thuisbatterij: { label: 'Thuisbatterij', icon: IC.battery, sub: 'Slaat u stroom op en word onafhankelijker' },
    laadpaal: { label: 'Laadpaal', icon: IC.charger, sub: 'Thuis laden — veilig en op eigen tempo' }
  };

  // gedeelde vraag: type woning
  var Q_WONING = { key: 'woning', label: 'Wat voor woning heeft u?', type: 'pills', required: true,
    options: ['Vrijstaande woning', 'Eengezinswoning', 'Appartement', 'Bedrijfspand'] };

  var branch = {
    thuisbatterij: [
      Q_WONING,
      { key: 'zon', label: 'Heeft u zonnepanelen?', type: 'pills', required: true, options: ['Ja', 'Nee', 'In aanvraag'] },
      ...
      { key: 'verbruik', label: 'Wat is uw jaarverbruik?', type: 'select', required: true,
        options: ['Minder dan 2.500 kWh', '2.500 – 3.500 kWh', '3.500 – 5.000 kWh', '5.000 – 8.000 kWh', 'Meer dan 8.000 kWh', 'Weet ik niet'] },
      { key: 'aansluiting', label: 'Wat voor aansluiting heeft u?', hint: 'staat op uw meterkast', type: 'pills', required: true, options: ['1-fase', '3-fase', 'Weet ik niet'] },
      { key: 'contract', label: 'Heeft u een dynamisch energiecontract?', type: 'pills', required: true, options: ['Ja', 'Nee', 'Interesse in'] },
    ...
    laadpaal: [
      Q_WONING,
      { key: 'aansluiting', label: 'Wat voor aansluiting heeft u?', hint: 'staat op uw meterkast', type: 'pills', required: true, options: ['1-fase', '3-fase', 'Weet ik niet'] },
      { key: 'kruipruimte', label: 'Heeft u een kruipruimte?', hint: 'handig voor het wegwerken van de kabel', type: 'pills', required: true, options: ['Ja', 'Nee', 'Weet ik niet'] },
      ...
      { key: 'slimmemeter', label: 'Heeft u een slimme meter?', type: 'pills', required: true, options: ['Ja', 'Nee', 'Weet ik niet'] }
    ]
  };
  ...
  function buildCalc(mount) {
    ...
    var FIRST = forced ? 1 : 0;                 // eerste zichtbare stap
    var stepIndex = FIRST;                      // 0=dienst 1=situatie 2=gegevens+adres
    var VISIBLE = forced ? [1, 2] : [0, 1, 2];  // dienst-stap vervalt bij vaste variant
    ...
    function render() {
      setProgress(false);
      if (stepIndex === 0) {
        if (forced) { stepIndex = 1; return render(); }
        return renderDienst();
      }
      if (stepIndex === 1) return renderSituatie();
      if (stepIndex === 2) return renderContact();
    }
    ...
    function bindNav(onNext) {
      ...
      if (back) back.addEventListener('click', function () { stepIndex = Math.max(FIRST, stepIndex - 1); render(); });
    ...
    function renderDienst() {
      ...
      wrap(
        '<h3>Waarvoor wilt u een offerte?</h3>' +
        '<p class="sub">Kies uw dienst. De volgende vragen passen we hierop aan.</p>' +
    ...
    function renderSituatie() {
      ...
      var html = '<h3>Vertel iets over uw situatie</h3>' +
        '<p class="sub">Hoe completer uw antwoord, hoe scherper we uw offerte maken.</p>';
      ...
      html += navHtml(stepIndex > FIRST, 'Naar gegevens');
      wrap(html);
    ...
    /* ---- Stap 3: gegevens + adres ---- */
    function renderContact() {
      wrap(
        '<h3>Uw gegevens</h3>' +
        '<p class="sub">Nog even uw gegevens en adres. U ontvangt binnen 1 werkdag een reactie, geheel vrijblijvend.</p>' +
        ...
        '<div class="row-2">' +
          '<div class="field"><label for="email">E-mailadres</label><input class="input" id="email" type="email" autocomplete="email" placeholder="u@voorbeeld.nl" value="' + esc(state.email || '') + '"></div>' +
          '<div class="field"><label for="tel">Telefoon <span class="hint">(optioneel)</span></label><input class="input" id="tel" type="tel" autocomplete="tel" placeholder="06 12 34 56 78" value="' + esc(state.tel || '') + '"></div>' +
        '</div>' +
        '<div class="row-2">' +
          '<div class="field"><label for="pc">Postcode</label><input class="input" id="pc" inputmode="text" autocomplete="postal-code" placeholder="1234 AB" value="' + esc(state.postcode) + '"></div>' +
          '<div class="field"><label for="hn">Huisnummer</label><input class="input" id="hn" inputmode="numeric" placeholder="12" value="' + esc(state.huisnr) + '"></div>' +
        '</div>' +
        '<div class="field"><label for="opm">Opmerkingen <span class="hint">(optioneel)</span></label><textarea class="input" id="opm" rows="3" placeholder="Bijzonderheden over uw situatie of planning">' + esc(state.opm || '') + '</textarea></div>' +
      ...
      bindNav(function () {
        ...
        if (!state.naam) return err('Vul uw naam in.');
        if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(state.email)) return err('Vul een geldig e-mailadres in.');
        if (state.tel && state.tel.replace(/\D/g, '').length < 8) return err('Vul een geldig telefoonnummer in, of laat het veld leeg.');
        if (!/^\d{4}\s?[A-Z]{2}$/.test(pc)) return err('Vul een geldige postcode in, bijvoorbeeld 1234 AB.');
        if (!hn) return err('Vul uw huisnummer in.');
        state.postcode = pc.replace(/^(\d{4})\s?([A-Z]{2})$/, '$1 $2');
        state.huisnr = hn;
        if (!akkoord) return err('Ga akkoord om uw aanvraag te versturen.');
        submit(nextBtn);
      });
    }
    ...
    function done() {
      ...
      body.innerHTML =
        '<div class="calc-done">' +
          ...
          '<p class="sub" style="margin-bottom:0">Uw aanvraag staat klaar. We nemen binnen 1 werkdag contact met u op met een offerte op maat.</p>' +
          '<div class="calc-summary">' + rows + '</div>' +
          '<p class="sub" style="font-size:.92rem">Nog een vraag? Mail ons gerust op ' +
            '<a class="link-arrow" style="display:inline-flex" href="mailto:' + CONTACT_EMAIL + '">' + CONTACT_EMAIL + '</a></p>' +
        '</div>';
    }
